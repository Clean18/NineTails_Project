...
public class Upgrade : MonoBehaviour
{
    ...
    {
        promotion.TryPromote(ref currentGrade, ref currentLevel, Warmth);
        if (mainUI == null) mainUI = UIManager.Instance.MainUI;
        mainUI.PlayerStatUI();
    }
    ...
    public void TryEnhance()
    {
        ...
        // 재화 확인
        if (Warmth < nextInfo.WarmthCost && !PlayerController.IsCheat)
        {
            Debug.Log("재화가 부족합니다");
            return;
        }
